...
if(isset($_GET['section'])){
	...
	$StrDisplay->Title_Form(customer,'1');
	...
	if($_POST['editdata']) {
		$StrDB->UpdateData($tb,"
			...
			,"where Cus_id='".$_POST['editdata']."'");
		$StrDisplay->EditSuccess();
		$StrDisplay->GoUrl('3',"index.php?action=".$action);
	}
	$rs=$StrDB->GetData($tb,"where Cus_id='".addslashes($_GET[id])."'");
	$StrDisplay->Title_Form(customer,'2');
	...
	$StrDisplay->TxtInputArea('ที่อยู่','txt-address',$rs[5]);
	$StrDisplay->TxtInput('เบอร์โทร','txt-tel',$rs[6]);
	$StrDisplay->TxtInput('อีเมล์','txt-email',$rs[7]);
 
	$StrDisplay->Button_Form('2',$_GET[id]);
	$StrDisplay->Close_Form();
	break;
	case 'del':
		$StrDB->DelData($tb," where Cus_id='".addslashes($_GET['id'])."' ");
		$StrDisplay->DelSuccess();
		$StrDisplay->GoUrl('3',"index.php?action=".$action);
	break;
	endswitch;
	}else{
		$StrDisplay->Title_Form(customer);
		...
		<a href=\"".WebUrl."admin/index.php?action=$_GET[action]&section=edit&id=$rs[Cus_id]\"><i class=\"icon-edit\" title=\"แก้ไข\"></i>
			</a> | ";
 		echo "<a href=\"".WebUrl."admin/index.php?action=$_GET[action]&section=del&id=$rs[Cus_id]\"  onClick=\"return confirm('ต้องการลบจริงหรือไม่ ?')\">";
